...
use crate::{
    hittable::hittable::{HittableList, Hittable},
    texture::ObjLoadingTexture,
    vec3::Point3, material::lambertian::Lambertian, bvh::BvhNode,
...
pub fn obj_mtl_load(name: &String) -> HittableList {
    ...
    for mat in m {
        if let Some(texture_name) = mat.diffuse_texture {
            ...
            if let Some(solid_texture) = mat.diffuse{
                textures.push(ObjLoadingTexture::new_solid_color((solid_texture[0],solid_texture[1],solid_texture[2])))
            }else{
    ...
    for mo in models {
        ...
        my_model.add(Box::new(BvhNode::new_nodes(&mut hittable_mo, 0,end , 0.0, 0.0)));      
    }
